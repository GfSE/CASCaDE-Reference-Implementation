...
/**
 * A simple class for IndexedDB to persist the package cache.
 */
class idb {
    /**
...
export const PackageCache = defineStore('packageCache', {
  ...
  actions: {
    ...
    /**
    * Update packages and persist them to IndexedDB.
    * @returns true if the packages were successfully persisted
    */
    async update(packages: APackage[]): Promise<boolean> {
        for(const pkg of packages) {
            ...
            // The following is made complicated to satisfy TypeScript's type checker,
            // which doesn't know that 'modified' is always defined as checked by the schema. 
            const existing = this.packages[index] as APackage;
            // Treat a missing 'modified' as the oldest possible date, so an
            // incoming package with a defined 'modified' always wins.
            const existingModified = existing.modified ?? '';
            const incomingModified = pkg.modified ?? '';
            if (existingModified < incomingModified) {
                const rev = existing.revision;
                if (rev && (!pkg.priorRevision || pkg.priorRevision.length === 0))
                    pkg.priorRevision = [rev];
